...
layui.define(["element", "layer", "jquery"], function (exports) {
  ...
  var tinyTab = {
    ...
    create: function (options) {
      ...
      options.maxTabNum = options.maxTabNum || 20;
      if ($(".layuiTiny-tab .layui-tab-title li").length >= options.maxTabNum) {
        layer.msg("Tab窗口已达到限定数量，请先关闭部分Tab");
        return false;
      }
      var ele = element;
      if (options.isIframe) ele = parent.layui.element;
      ele.tabAdd("layuitinyTab", {
        title:
          '<span class="layuiTiny-tab-active"></span><span>' +
          options.title +
      ...
      $(".layuiTiny-menu-left").attr("layuiTiny-tab-tag", "add");
      sessionStorage.setItem("layuiTinymenu_" + options.tabId, options.title);
    },
    ...
    change: function (tabId) {
      element.tabChange("layuitinyTab", tabId);
    },
    ...
    delete: function (tabId, isParent) {
      ...
      $(
        ".layuiTiny-tab .layui-tab-title .layui-unselect.layui-tab-bar"
      ).remove();

      if (isParent === true) {
        parent.layui.element.tabDelete("layuitinyTab", tabId);
      } else {
        element.tabDelete("layuitinyTab", tabId);
      }
    },
    ...
    openNewTabByIframe: function (options) {
      ...
      if (options.href === null || options.href === undefined)
        options.href = new Date().getTime();
      var checkTab = tinyTab.check(options.href, true);
      if (!checkTab) {
        tinyTab.create({
          tabId: options.href,
          href: options.href,
      ...
      parent.layui.element.tabChange("layuitinyTab", options.href);
    ...
    deleteCurrentByIframe: function () {
      var ele = $(
        ".layuiTiny-tab .layui-tab-title li.layui-this",
        parent.document
      );
      if (ele.length > 0) {
    ...
    closeTabRignMenu: function () {
      $(".layuiTiny-tab-mousedown").remove();
      $(".layuiTiny-tab-make").remove();
    },
    ...
    searchMenu: function (href, menuList) {
      ...
      for (key in menuList) {
        ...
        if (item.child) {
          newMenu = tinyTab.searchMenu(href, item.child);
          if (newMenu) {
            menu = newMenu;
            break;
          }
    ...
    listen: function (options) {
       ...
       * 打开新窗口
       */
      $("body").on("click", "[layuiTiny-href]", function () {
        var loading = layer.load(0, { shade: false, time: 2 * 1000 });
        var tabId = $(this).attr("layuiTiny-href"),
          href = $(this).attr("layuiTiny-href"),
          title = $(this).text(),
          target = $(this).attr("target");

        var el = $("[layuiTiny-href='" + href + "']", ".layuiTiny-menu-left");
        ...
        var checkTab = tinyTab.check(tabId);
        if (!checkTab) {
          tinyTab.create({
            tabId: tabId,
            href: href,
       ...
       * 在iframe子菜单上打开新窗口
       */
      $("body").on("click", "[layuiTiny-content-href]", function () {
        var loading = parent.layer.load(0, { shade: false, time: 2 * 1000 });
        var tabId = $(this).attr("layuiTiny-content-href"),
          href = $(this).attr("layuiTiny-content-href"),
        ...
        if (tabId === null || tabId === undefined) tabId = new Date().getTime();
        var checkTab = tinyTab.check(tabId, true);
        if (!checkTab) {
          tinyTab.create({
            tabId: tabId,
            href: href,
        ...
        parent.layui.element.tabChange("layuitinyTab", tabId);
        parent.layer.close(loading);
      });
      ...
      $("body").on(
        "click",
        ".layuiTiny-tab .layui-tab-title .layui-tab-close",
        function () {
          var loading = layer.load(0, { shade: false, time: 2 * 1000 });
          var $parent = $(this).parent();
          var tabId = $parent.attr("lay-id");
          if (tabId !== undefined || tabId !== null) {
            tinyTab.delete(tabId);
          }
          layer.close(loading);
        }
      );
       ...
       * 选项卡操作
       */
      $("body").on("click", "[layuiTiny-tab-close]", function () {
        var loading = layer.load(0, { shade: false, time: 2 * 1000 });
        var closeType = $(this).attr("layuiTiny-tab-close");
        $(".layuiTiny-tab .layui-tab-title li").each(function () {
          var tabId = $(this).attr("lay-id");
          var id = $(this).attr("id");
          var isCurrent = $(this).hasClass("layui-this");
          if (id !== "layuiTinyHomeTabId") {
            if (closeType === "all") {
              tinyTab.delete(tabId);
            } else {
      ...
      $("body").on(
        "mousedown",
        ".layuiTiny-tab .layui-tab-title li",
        function (e) {
          var left =
              $(this).offset().left -
              $(".layuiTiny-tab ").offset().left +
              $(this).width() / 2,
            tabId = $(this).attr("lay-id");
          if (e.which === 3) {
            tinyTab.openTabRignMenu(tabId, left);
          }
        }
      );
      ...
      $("body").on(
        "click",
        ".layui-body,.layui-header,.layuiTiny-menu-left,.layuiTiny-tab-make",
        function () {
          tinyTab.closeTabRignMenu();
        }
      );
       ...
       * tab右键选项卡操作
       */
      $("body").on("click", "[layuiTiny-tab-menu-close]", function () {
        var loading = layer.load(0, { shade: false, time: 2 * 1000 });
        var closeType = $(this).attr("layuiTiny-tab-menu-close"),
          currentTabId = $(".layuiTiny-tab-mousedown").attr("data-tab-id");
        $(".layuiTiny-tab .layui-tab-title li").each(function () {
          var tabId = $(this).attr("lay-id");
          var id = $(this).attr("id");
          if (id !== "layuiTinyHomeTabId") {
    ...
    listenSwitch: function (options) {
      ...
      element.on("tab(" + options.filter + ")", function (data) {
        ...
        // 判断是否为新增窗口
        if ($(".layuiTiny-menu-left").attr("layuiTiny-tab-tag") === "add") {
          $(".layuiTiny-menu-left").attr("layuiTiny-tab-tag", "no");
        } else {
          $("[layuiTiny-href]").parent().removeClass("layui-this");
    ...
    listenHash: function (options) {
      ...
      // 判断是否为右侧菜单
      var menu = tinyTab.searchMenu(tabId, options.menuList);
      if (menu !== undefined) {
        tinyTab.create({
          tabId: tabId,
          href: tabId,
      ...
      $("[layuiTiny-content-href]").each(function () {
        if ($(this).attr("layuiTiny-content-href") === tabId) {
      ...
      // 既不是右侧菜单、快捷菜单,就直接打开
      var title =
        sessionStorage.getItem("layuiTinymenu_" + tabId) === null
          ? tabId
          : sessionStorage.getItem("layuiTinymenu_" + tabId);
      tinyTab.create({
        tabId: tabId,
        href: tabId,
        title: title,
     ...
     */
    listenRoll: function () {
      $(".layuiTiny-tab-roll-left").click(function () {
        tinyTab.rollClick("left");
      });
      $(".layuiTiny-tab-roll-right").click(function () {
        tinyTab.rollClick("right");
      });
    },
    ...
    listenSwitchSingleModule: function (tabId) {
      $("[layuiTiny-href]").each(function () {
        if ($(this).attr("layuiTiny-href") === tabId) {
          // 自动展开菜单栏
          var addMenuClass = function ($element, type) {
            if (type === 1) {
              ...
              if (
                ...
                $(".layuiTiny-header-menu li").attr("class", "layui-nav-item");
              } else {
          ...
          var addMenuClass = function ($element, type) {
            if (type === 1) {
              ...
              if (
                ...
                $(".layuiTiny-menu-left .layui-nav.layui-nav-tree").attr(
                  "class",
    ...
    rollPosition: function () {
      var $tabTitle = $(".layuiTiny-tab  .layui-tab-title");
      var autoLeft = 0;
